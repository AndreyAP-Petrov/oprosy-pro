...
var DATA_DIR = process.env.OPROSY_DATA_DIR || (path.basename(__dirname) === "current" ? path.resolve(__dirname, "..", "data") : LEGACY_DATA_DIR);
...
var MAX_JSON_BYTES = 8e6;
var LOGIN_WINDOW_MS = 10 * 60 * 1e3;
var LOGIN_MAX_ATTEMPTS = 12;
if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });
migrateLegacyDataDir();
var MASTER_KEY = getOrCreateMasterKey();
var db = loadDb();
normalizeDb(db);
ensureOwner(db);
saveDb();
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname.startsWith("/api/")) {
      ...
      if (url.pathname === "/api/auth") {
        const result = await handleAuth(body, req, res);
        return sendJson(res, result.status || 200, result.body);
      }
      if (url.pathname === "/api/app") {
        const result = handleApp(body, req);
        return sendJson(res, result.status || 200, result.body);
      }
    }
    ...
    return sendJson(res, e.statusCode || 500, { error: e.message || String(e) });
  }
});
server.listen(PORT, "0.0.0.0", () => {
  console.log(`\u041E\u043F\u0440\u043E\u0441\u044B PRO \u0437\u0430\u043F\u0443\u0449\u0435\u043D: http://localhost:${PORT}`);
  const owner = db.users[OWNER_EMAIL];
  if (owner && !owner.passwordHash && owner.activationHash) {
...
function migrateLegacyDataDir() {
  ...
  const candidates = [];
  const directLegacy = path.join(LEGACY_DATA_DIR, "db.json");
  if (fs.existsSync(directLegacy)) candidates.push(directLegacy);
  if (path.basename(__dirname) === "current") {
    ...
    const walk = (dir, depth) => {
      ...
      for (const entry of entries) {
        ...
        if (entry.name === "node_modules" || entry.name === ".git" || path.resolve(full) === path.resolve(DATA_DIR)) continue;
        if (entry.isDirectory()) walk(full, depth + 1);
        else if (entry.isFile() && entry.name === "db.json" && path.resolve(full) !== path.resolve(DB_PATH)) candidates.push(full);
      }
    };
    walk(projectRoot, 0);
  }
  const unique = [...new Set(candidates)].filter((f) => fs.existsSync(f));
  unique.sort((a, b) => {
    try {
  ...
  try {
    ...
    fs.chmodSync(DB_PATH, 384);
    console.log(`[MIGRATION] \u0421\u0442\u0430\u0440\u0430\u044F \u0431\u0430\u0437\u0430 \u043F\u0435\u0440\u0435\u043D\u0435\u0441\u0435\u043D\u0430 \u0438\u0437 ${source} \u0432 \u043F\u043E\u0441\u0442\u043E\u044F\u043D\u043D\u043E\u0435 \u0445\u0440\u0430\u043D\u0438\u043B\u0438\u0449\u0435: ${DATA_DIR}`);
  } catch (e) {
    console.error("[MIGRATION] \u041D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043F\u0435\u0440\u0435\u043D\u0435\u0441\u0442\u0438 \u0441\u0442\u0430\u0440\u0443\u044E \u0431\u0430\u0437\u0443:", e.message);
  }
}
async function handleAuth(body, req, res) {
  cleanup();
  const action = String(body.action || "");
  if (action === "activateAccount") {
    const email = normEmail(body.email);
    const user = db.users[email];
    if (!user || !user.activationHash || !verifyOpaqueCode(body.activationCode, user.activationHash)) {
      return bad("\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 email \u0438\u043B\u0438 \u043A\u043E\u0434 \u0430\u043A\u0442\u0438\u0432\u0430\u0446\u0438\u0438", 401);
    }
    if (user.activationExp && user.activationExp < Date.now()) return bad("\u041A\u043E\u0434 \u0430\u043A\u0442\u0438\u0432\u0430\u0446\u0438\u0438 \u0438\u0441\u0442\u0451\u043A. \u041F\u043E\u043F\u0440\u043E\u0441\u0438\u0442\u0435 \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440\u0430 \u0432\u044B\u0434\u0430\u0442\u044C \u043D\u043E\u0432\u044B\u0439.", 401);
    assertPassword(body.newPassword);
    const name = String(body.name || "").trim();
    if (!name) return bad("\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u0438\u043C\u044F", 400);
    user.name = name;
    user.passwordHash = hashPassword(body.newPassword);
    user.mustChangePassword = false;
    user.activationHash = "";
    user.activationExp = 0;
    user.activatedAt = (/* @__PURE__ */ new Date()).toISOString();
    if (email === OWNER_EMAIL) {
      try {
        if (fs.existsSync(OWNER_CODE_PATH)) fs.unlinkSync(OWNER_CODE_PATH);
      } catch (_) {
      }
    }
    if (!user.twoFactorEnabled) {
      const secret = base32Secret();
      user.pendingTotpSecretEnc = encryptSecret(secret);
      const tempToken = createTemp(email, "setup_2fa");
      saveDb();
      return ok({
        require2faSetup: true,
        tempToken,
        secret,
        qrDataUrl: await qrDataUrl(otpauth(email, secret))
      });
    }
    saveDb();
    return ok({ activated: true, requireLogin: true });
  }
  if (action === "login") {
    const email = normEmail(body.email);
    ...
    const user = db.users[email];
    if (!body.password || String(body.password).length < 5) {
      recordLoginFailure(ip, email);
      return bad("\u041F\u0430\u0440\u043E\u043B\u044C \u0434\u043E\u043B\u0436\u0435\u043D \u0431\u044B\u0442\u044C \u043C\u0438\u043D\u0438\u043C\u0443\u043C 5 \u0441\u0438\u043C\u0432\u043E\u043B\u043E\u0432", 400);
    }
    if (!user || !user.passwordHash || !verifyPassword(body.password, user.passwordHash)) {
      recordLoginFailure(ip, email);
      return bad("\u041D\u0435\u0432\u0435\u0440\u043D\u0430\u044F \u043F\u043E\u0447\u0442\u0430 \u0438\u043B\u0438 \u043F\u0430\u0440\u043E\u043B\u044C", 401);
    }
    if (user.disabled) return bad("\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u0437\u0430\u0431\u043B\u043E\u043A\u0438\u0440\u043E\u0432\u0430\u043D", 403);
    if (user.mustChangePassword) {
      const tempToken = createTemp(email, "first_login");
      saveDb();
      return ok({ requireFirstLogin: true, tempToken, name: user.name || "" });
    }
    if (!user.twoFactorEnabled) {
      const secret2 = base32Secret();
      user.pendingTotpSecretEnc = encryptSecret(secret2);
      const tempToken = createTemp(email, "setup_2fa");
      saveDb();
      return ok({
        require2faSetup: true,
        tempToken,
        secret: secret2,
        qrDataUrl: await qrDataUrl(otpauth(email, secret2))
      });
    }
    const secret = decryptSecret(user.totpSecretEnc || "");
    if (!verifyTotp(body.totp, secret)) {
      recordLoginFailure(ip, email);
      return bad("\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 2FA \u043A\u043E\u0434", 401);
    }
    clearLoginFailures(ip, email);
    const rawToken = createSession(email);
    setSessionCookie(req, res, rawToken);
    saveDb();
    return ok({ user: publicUser(user) });
  }
  if (action === "completeFirstLogin") {
    const temp = getTemp(body.tempToken, "first_login");
    if (!temp) return bad("\u0412\u0440\u0435\u043C\u0435\u043D\u043D\u0430\u044F \u0441\u0435\u0441\u0441\u0438\u044F \u0438\u0441\u0442\u0435\u043A\u043B\u0430", 401);
    assertPassword(body.newPassword);
    const user = db.users[temp.email];
    const name = String(body.name || "").trim();
    if (!name) return bad("\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u0438\u043C\u044F", 400);
    user.name = name;
    user.passwordHash = hashPassword(body.newPassword);
    ...
    const secret = base32Secret();
    user.pendingTotpSecretEnc = encryptSecret(secret);
    changeTempType(body.tempToken, "setup_2fa");
    saveDb();
    return ok({ tempToken: body.tempToken, secret, qrDataUrl: await qrDataUrl(otpauth(user.email, secret)) });
  }
  if (action === "enable2fa") {
    const temp = getTemp(body.tempToken, "setup_2fa");
    if (!temp) return bad("\u0412\u0440\u0435\u043C\u0435\u043D\u043D\u0430\u044F \u0441\u0435\u0441\u0441\u0438\u044F \u0438\u0441\u0442\u0435\u043A\u043B\u0430", 401);
    const user = db.users[temp.email];
    const secret = decryptSecret(user.pendingTotpSecretEnc || "");
    if (!secret || !verifyTotp(body.code, secret)) return bad("\u041D\u0435\u0432\u0435\u0440\u043D\u044B\u0439 2FA \u043A\u043E\u0434", 401);
    user.totpSecretEnc = encryptSecret(secret);
    user.pendingTotpSecretEnc = "";
    user.twoFactorEnabled = true;
    deleteTemp(body.tempToken);
    const rawToken = createSession(user.email);
    setSessionCookie(req, res, rawToken);
    saveDb();
    return ok({ user: publicUser(user) });
  }
  if (action === "logout") {
    ...
    if (raw) delete db.sessions[hashToken(raw)];
    clearSessionCookie(req, res);
    saveDb();
    return ok({ ok: true });
  }
  const actor = requireSessionFromRequest(req);
  if (action === "me") return ok({ user: publicUser(actor) });
  if (action === "updateProfile") {
    const user = db.users[actor.email];
    const name = String(body.name || "").trim();
    if (!name) return bad("\u0412\u0432\u0435\u0434\u0438\u0442\u0435 \u0438\u043C\u044F", 400);
    user.name = name;
    if (body.newPassword) {
  ...
  requireAdmin(actor);
  if (action === "listUsers") {
    return ok({ users: Object.values(db.users).map(adminUser).sort((a, b) => a.email.localeCompare(b.email)) });
  }
  if (action === "createUser") {
    const email = normEmail(body.email);
    const name = String(body.name || "").trim();
    if (!email) return bad("Email \u043E\u0431\u044F\u0437\u0430\u0442\u0435\u043B\u0435\u043D", 400);
    if (!name) return bad("\u0418\u043C\u044F \u043E\u0431\u044F\u0437\u0430\u0442\u0435\u043B\u044C\u043D\u043E", 400);
    if (db.users[email]) return bad("\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u0443\u0436\u0435 \u0441\u0443\u0449\u0435\u0441\u0442\u0432\u0443\u0435\u0442", 400);
    const role = "owner";
    const activationCode = generateActivationCode();
    db.users[email] = makePendingUser(email, role, name, activationCode);
    saveDb();
    return ok({ ok: true, activationCode, activationExpiresAt: db.users[email].activationExp });
  }
  if (action === "resetPassword") {
    const email = normEmail(body.email);
    const user = db.users[email];
    if (!user) return bad("\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D", 404);
    if (user.role === "owner" && actor.role !== "owner") return bad("Owner \u043C\u043E\u0436\u0435\u0442 \u043C\u0435\u043D\u044F\u0442\u044C \u0442\u043E\u043B\u044C\u043A\u043E owner", 403);
    const activationCode = generateActivationCode();
    user.passwordHash = "";
    user.mustChangePassword = false;
    user.activationHash = hashOpaqueCode(activationCode);
    user.activationExp = Date.now() + ACTIVATION_TTL_MS;
    invalidateUserSessions(email);
    saveDb();
    return ok({ ok: true, activationCode, activationExpiresAt: user.activationExp });
  }
  if (action === "reset2fa") {
    const email = normEmail(body.email);
    const user = db.users[email];
  ...
  if (action === "setRole") {
    ...
    user.role = "owner";
    saveDb();
    return ok({ ok: true, role: "owner" });
  }
  if (action === "deleteUser") {
    const email = normEmail(body.email);
    const user = db.users[email];
    if (!user) return bad("\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D", 404);
    if (email === OWNER_EMAIL) return bad("Нельзя удалить главного владельца системы", 403);
    delete db.users[email];
    for (const [id, stat] of Object.entries(db.statistics)) {
      if (stat.ownerEmail === email) delete db.statistics[id];
    }
    invalidateUserSessions(email);
    saveDb();
    return ok({ ok: true });
...
function handleApp(body, req) {
  cleanup();
  const actor = requireSessionFromRequest(req);
  const action = String(body.action || "");
  if (action === "listWorkspace") {
    return ok(buildWorkspace(actor));
  }
  ...
  if (action === "deleteReport") {
    ...
    if (!existing) return bad("\u0421\u0442\u0430\u0442\u0438\u0441\u0442\u0438\u043A\u0430 \u043D\u0435 \u043D\u0430\u0439\u0434\u0435\u043D\u0430", 404);
    if (existing.ownerEmail !== actor.email) return bad("\u0423\u0434\u0430\u043B\u0438\u0442\u044C \u0441\u0442\u0430\u0442\u0438\u0441\u0442\u0438\u043A\u0443 \u043C\u043E\u0436\u0435\u0442 \u0442\u043E\u043B\u044C\u043A\u043E \u0435\u0451 \u0432\u043B\u0430\u0434\u0435\u043B\u0435\u0446", 403);
    delete db.statistics[id];
    saveDb();
    return ok({ ok: true });
  }
  if (action === "importReports") {
    ...
    saveDb();
    return ok({ ok: true, imported: reports.length });
  }
  if (action === "importLegacyState") {
    if (actor.role !== "owner") return bad("\u041F\u0435\u0440\u0435\u043D\u043E\u0441 \u0441\u0442\u0430\u0440\u044B\u0445 \u043E\u0431\u0449\u0438\u0445 \u0434\u0430\u043D\u043D\u044B\u0445 \u0434\u043E\u0441\u0442\u0443\u043F\u0435\u043D \u0442\u043E\u043B\u044C\u043A\u043E owner", 403);
    if (db.legacyStateImportedAt) return ok({ ok: true, imported: 0, alreadyImported: true });
    const legacyUsers = Array.isArray(body.legacyState?.users) ? body.legacyState.users.slice(0, 100) : [];
    let imported = 0;
    const mappings = [];
    for (const oldUser of legacyUsers) {
      const oldName = String(oldUser?.name || "").trim();
      const target = Object.values(db.users).find((u) => !!u.passwordHash && String(u.name || "").trim().toLowerCase() === oldName.toLowerCase()) || actor;
      let count = 0;
      ...
      if (count) mappings.push({ from: oldName || "\u0411\u0435\u0437 \u0438\u043C\u0435\u043D\u0438", to: target.name || target.email, count });
    }
    db.legacyStateImportedAt = (/* @__PURE__ */ new Date()).toISOString();
    saveDb();
    return ok({ ok: true, imported, mappings });
  }
  return bad("Unknown action", 400);
}
function buildWorkspace(actor) {
  const users = Object.values(db.users).filter((u) => !!u.passwordHash && !u.disabled).sort((a, b) => (a.name || a.email).localeCompare(b.name || b.email, "ru")).map((u) => ({
    id: userPublicId(u.email),
    name: u.name || "\u041F\u043E\u043B\u044C\u0437\u043E\u0432\u0430\u0442\u0435\u043B\u044C",
    reports: Object.values(db.statistics).filter((r) => r.ownerEmail === u.email && canViewReport(actor, r)).sort((a, b) => String(b.updatedAt || "").localeCompare(String(a.updatedAt || ""))).map((r) => clientReport(r, actor))
  }));
  return { users, currentUser: publicUser(actor) };
}
function canViewReport(actor, report) {
...
function defaultReportRows() {
  return DEFAULT_REPORT_COLLECTORS.map((social) => ({
...
function normalizeReport(report, ownerEmail) {
  ...
  return {
    ...
    customColumns: (Array.isArray(report.customColumns) ? report.customColumns : []).slice(0, 50).map((c) => ({
      ...
      width: clampNumber(c.width, 55, 600, 140),
      ...
      decimals: clampNumber(c.decimals, 0, 6, 2)
    })),
    columnWidths: report.columnWidths && typeof report.columnWidths === "object" ? Object.fromEntries(Object.entries(report.columnWidths).slice(0, 100).map(([k, v]) => [String(k).slice(0, 100), clampNumber(v, 55, 600, 120)])) : {},
    rows: ((Array.isArray(report.rows) && report.rows.length ? report.rows : defaultReportRows())).slice(0, 500).map((row) => normalizeRow(row, vat)),
    createdAt: report.createdAt || (/* @__PURE__ */ new Date()).toISOString(),
    updatedAt: report.updatedAt || (/* @__PURE__ */ new Date()).toISOString()
  };
}
function normalizeRow(row, vat) {
  const coef = safeNumber(vat) || 1.22;
  let spentNet;
  if (row.spentNet !== void 0 && row.spentNet !== null) spentNet = safeNumber(row.spentNet);
  else if (row.spentVat !== void 0 && row.spentVat !== null) spentNet = coef > 0 ? safeNumber(row.spentVat) / coef : safeNumber(row.spentVat);
  else spentNet = safeNumber(row.spent);
  const customFields = row.customFields && typeof row.customFields === "object" ? Object.fromEntries(Object.entries(row.customFields).slice(0, 50).map(([k, v]) => [String(k).slice(0, 80), String(v ?? "").slice(0, 500)])) : {};
  return {
    social: String(row.social || "Новая строка").slice(0, 150),
    impressions: clampNumber(row.impressions, 0, 1e12, 0),
    clicks: clampNumber(row.clicks, 0, 1e12, 0),
    collected: clampNumber(row.collected, 0, 1e12, 0),
    spentNet: clampNumber(spentNet, 0, 1e15, 0),
    need: clampNumber(row.need, 0, 1e12, 0),
    customFields,
    status: ["РАБОТАЕТ", "СОБРАЛИ", "НЕ РАБОТАЕТ"].includes(row.status) ? row.status : "РАБОТАЕТ"
  };
}
function loadDb() {
  if (!fs.existsSync(DB_PATH)) return { version: 2, users: {}, sessions: {}, temp: {}, statistics: {} };
  try {
    return JSON.parse(fs.readFileSync(DB_PATH, "utf8"));
  } catch (e) {
    console.error("\u041D\u0435 \u0443\u0434\u0430\u043B\u043E\u0441\u044C \u043F\u0440\u043E\u0447\u0438\u0442\u0430\u0442\u044C db.json:", e.message);
    return { version: 2, users: {}, sessions: {}, temp: {}, statistics: {} };
  }
}
function normalizeDb(target) {
  ...
  if (!target.securityVersion || target.securityVersion < 2) {
    target.sessions = {};
    target.temp = {};
    target.securityVersion = 2;
  }
  const usedUserIds = new Set();
  for (const user of Object.values(target.users)) {
    user.email = normEmail(user.email);
    // В этой закрытой версии сервис рассчитан на небольшую команду:
    // каждый зарегистрированный аккаунт имеет полный уровень OWNER.
    user.role = "owner";

    let persistentId = String(user.id || "").trim();
    if (!/^usr_[a-z0-9_-]{8,64}$/i.test(persistentId) || usedUserIds.has(persistentId)) {
      do { persistentId = generateUserId(); } while (usedUserIds.has(persistentId));
    }
    user.id = persistentId;
    usedUserIds.add(persistentId);

    if (user.totpSecret && !user.totpSecretEnc) user.totpSecretEnc = encryptSecret(user.totpSecret);
    if (user.pendingTotpSecret && !user.pendingTotpSecretEnc) user.pendingTotpSecretEnc = encryptSecret(user.pendingTotpSecret);
    delete user.totpSecret;
    delete user.pendingTotpSecret;
    user.totpSecretEnc = user.totpSecretEnc || "";
    user.pendingTotpSecretEnc = user.pendingTotpSecretEnc || "";
    user.activationHash = user.activationHash || "";
    user.activationExp = Number(user.activationExp || 0);
  }
  const normalizedStats = {};
  for (const [id, report] of Object.entries(target.statistics)) {
    try {
      normalizedStats[String(id)] = normalizeReport(report, report.ownerEmail);
    } catch (_) {
    }
  }
  target.statistics = normalizedStats;
}
function saveDb() {
  cleanup();
  const tmpPath = DB_PATH + ".tmp";
  fs.writeFileSync(tmpPath, JSON.stringify(db, null, 2), { encoding: "utf8", mode: 384 });
...
function ensureOwner(target) {
  if (target.users[OWNER_EMAIL]) {
    target.users[OWNER_EMAIL].role = "owner";
    if (!target.users[OWNER_EMAIL].id) target.users[OWNER_EMAIL].id = generateUserId();
    return;
  }
  let activationCode = process.env.OPROSY_OWNER_ACTIVATION_CODE || "";
  if (!activationCode && fs.existsSync(OWNER_CODE_PATH)) activationCode = fs.readFileSync(OWNER_CODE_PATH, "utf8").trim();
  if (!activationCode) {
...
function makePendingUser(email, role, name, activationCode) {
  return {
    ...
    activationHash: hashOpaqueCode(activationCode),
    activationExp: Date.now() + ACTIVATION_TTL_MS,
...
function userPublicId(email) {
  const user = db?.users?.[normEmail(email)];
  return user?.id || ("legacy_" + crypto.createHash("sha256").update(normEmail(email)).digest("hex").slice(0, 20));
}
function publicUser(u) {
  return {
    id: u.id || userPublicId(u.email),
    email: u.email,
    name: u.name || u.email,
    role: "owner",
    isPrimaryOwner: normEmail(u.email) === OWNER_EMAIL,
    activated: !!u.passwordHash,
    needsActivation: !u.passwordHash && !!u.activationHash,
...
function createSession(email) {
  const raw = makeToken();
  db.sessions[hashToken(raw)] = { email: normEmail(email), exp: Date.now() + SESSION_TTL_MS };
  return raw;
}
function requireSessionFromRequest(req) {
  const raw = getSessionCookie(req);
  const s = raw ? db.sessions[hashToken(raw)] : null;
  if (!s || s.exp < Date.now()) throw Object.assign(new Error("\u0421\u0435\u0441\u0441\u0438\u044F \u0438\u0441\u0442\u0435\u043A\u043B\u0430"), { statusCode: 401 });
  const user = db.users[s.email];
  if (!user || user.disabled || !user.passwordHash) {
    throw Object.assign(new Error("Доступ только для зарегистрированных и активированных пользователей"), { statusCode: 401 });
  }
  return user;
}
function invalidateUserSessions(email) {
  for (const [hash, s] of Object.entries(db.sessions)) if (s.email === email) delete db.sessions[hash];
}
function createTemp(email, type) {
  const raw = makeToken();
  db.temp[hashToken(raw)] = { email: normEmail(email), type, exp: Date.now() + TEMP_TTL_MS };
  return raw;
}
function getTemp(raw, type) {
  const temp = raw ? db.temp[hashToken(raw)] : null;
  if (!temp || temp.type !== type || temp.exp < Date.now()) return null;
  return temp;
}
function changeTempType(raw, type) {
  const key = hashToken(raw);
  if (db.temp[key]) {
    db.temp[key].type = type;
    db.temp[key].exp = Date.now() + TEMP_TTL_MS;
  }
}
function deleteTemp(raw) {
  delete db.temp[hashToken(raw)];
}
function cleanup() {
  const now = Date.now();
  for (const [t, s] of Object.entries(db.sessions || {})) if (s.exp < now) delete db.sessions[t];
  for (const [t, s] of Object.entries(db.temp || {})) if (s.exp < now) delete db.temp[t];
}
function getOrCreateMasterKey() {
  if (process.env.DATA_ENCRYPTION_KEY) return crypto.createHash("sha256").update(process.env.DATA_ENCRYPTION_KEY).digest();
...
function assertPassword(p) {
  if (!p || String(p).length < 5) throw Object.assign(new Error("\u041F\u0430\u0440\u043E\u043B\u044C \u0434\u043E\u043B\u0436\u0435\u043D \u0431\u044B\u0442\u044C \u043C\u0438\u043D\u0438\u043C\u0443\u043C 5 \u0441\u0438\u043C\u0432\u043E\u043B\u043E\u0432"), { statusCode: 400 });
  if (String(p).length > 256) throw Object.assign(new Error("\u041F\u0430\u0440\u043E\u043B\u044C \u0441\u043B\u0438\u0448\u043A\u043E\u043C \u0434\u043B\u0438\u043D\u043D\u044B\u0439"), { statusCode: 400 });
}
...
function verifyPassword(password, stored) {
  ...
  try {
    ...
    if (parts[0] === "pbkdf2" && parts.length === 3) {
      ...
      const check = crypto.pbkdf2Sync(String(password), salt, 21e4, 32, "sha256").toString("hex");
...
function hashOpaqueCode(raw) {
  return crypto.createHash("sha256").update(String(raw || "").trim().toUpperCase()).digest("hex");
}
function verifyOpaqueCode(raw, hash) {
  if (!raw || !hash) return false;
  const a = Buffer.from(hashOpaqueCode(raw), "hex");
...
function verifyTotp(code, secret) {
  ...
  for (let drift = -1; drift <= 1; drift++) if (hotp(secret, counter + drift) === code) return true;
  return false;
}
function otpauth(email, secret) {
  return `otpauth://totp/OprosyPRO:${encodeURIComponent(email)}?secret=${secret}&issuer=OprosyPRO&period=30&digits=6`;
}
...
function checkLoginRate(ip, email) {
  ...
  if (!item) return;
  if (Date.now() - item.start > LOGIN_WINDOW_MS) {
    loginAttempts.delete(key);
    return;
  }
  if (item.count >= LOGIN_MAX_ATTEMPTS) throw Object.assign(new Error("\u0421\u043B\u0438\u0448\u043A\u043E\u043C \u043C\u043D\u043E\u0433\u043E \u043F\u043E\u043F\u044B\u0442\u043E\u043A \u0432\u0445\u043E\u0434\u0430. \u041F\u043E\u0432\u0442\u043E\u0440\u0438\u0442\u0435 \u043F\u043E\u0437\u0436\u0435."), { statusCode: 429 });
}
function recordLoginFailure(ip, email) {
  const key = loginKey(ip, email);
  const now = Date.now();
  let item = loginAttempts.get(key);
  if (!item || now - item.start > LOGIN_WINDOW_MS) item = { count: 0, start: now };
  item.count += 1;
...
function getSessionCookie(req) {
  return parseCookies(req).oprosy_session || "";
}
...
function setSessionCookie(req, res, token) {
  ...
  res.setHeader("Set-Cookie", `oprosy_session=${encodeURIComponent(token)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=${Math.floor(SESSION_TTL_MS / 1e3)}${secure}`);
}
function clearSessionCookie(req, res) {
  const secure = isHttps(req) ? "; Secure" : "";
  res.setHeader("Set-Cookie", `oprosy_session=; HttpOnly; SameSite=Strict; Path=/; Max-Age=0${secure}`);
}
function enforceSameOrigin(req) {
  ...
  try {
    ...
    if (originUrl.host !== host) throw Object.assign(new Error("\u0417\u0430\u043F\u0440\u043E\u0441 \u0441 \u0434\u0440\u0443\u0433\u043E\u0433\u043E \u0438\u0441\u0442\u043E\u0447\u043D\u0438\u043A\u0430 \u043E\u0442\u043A\u043B\u043E\u043D\u0451\u043D"), { statusCode: 403 });
  } catch (e) {
    if (e.statusCode) throw e;
    throw Object.assign(new Error("\u041D\u0435\u043A\u043E\u0440\u0440\u0435\u043A\u0442\u043D\u044B\u0439 Origin"), { statusCode: 403 });
  }
}
function setSecurityHeaders(res) {
  ...
  res.setHeader("Content-Security-Policy", "default-src 'self'; script-src 'self' https://cdn.jsdelivr.net; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self'; frame-ancestors 'none'; base-uri 'self'; form-action 'self'");
}
...
function serveStatic(req, res, pathname) {
  ...
  if (!fs.existsSync(fullPath) || fs.statSync(fullPath).isDirectory()) fullPath = path.join(PUBLIC_DIR, "index.html");
...
function requireAdmin(user) {
  if (user.role !== "admin" && user.role !== "owner") throw Object.assign(new Error("\u041D\u0443\u0436\u043D\u044B \u043F\u0440\u0430\u0432\u0430 \u0430\u0434\u043C\u0438\u043D\u0438\u0441\u0442\u0440\u0430\u0442\u043E\u0440\u0430"), { statusCode: 403 });
}
...
function generateUserId() {
  return "usr_" + crypto.randomBytes(9).toString("base64url").replace(/[^a-zA-Z0-9_-]/g, "").toLowerCase();
}
